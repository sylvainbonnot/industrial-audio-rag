...
def create_demo():
    ...
    with gr.Blocks(
        ...
        **Retrieval-Augmented Generation for Industrial Audio Analysis**
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    max_results = gr.Slider(
                        minimum=1, maximum=10, value=5, step=1, label="Maximum results to retrieve"
                    )
            ...
            with gr.Column(scale=1):
                ...
                system_status = gr.JSON(value=system_info, label="System Status")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                metrics_output = gr.Markdown(label="Performance Metrics", elem_classes=["metrics"])
        ...
        This system uses **Retrieval-Augmented Generation (RAG)** to answer questions about
        industrial audio data:
        ...
        **Dataset:** DCASE 2024 Task-2 - Unsupervised Anomalous Sound Detection for Machine
        ...
        for i, btn in enumerate(sample_buttons):
            btn.click(fn=lambda sample=SAMPLE_QUERIES[i]: sample, outputs=query_input)

        # Auto-refresh system info every 30 seconds
        demo.load(fn=get_system_info, outputs=system_status, every=30)
