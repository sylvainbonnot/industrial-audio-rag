...
class RAGEvaluator:
    ...
    def validate_response_schema(self, response: Dict[str, Any], schema: Dict[str, Any]) -> List[str]:
    ...
    def calculate_answer_quality_score(self, answer: str, expected_length: Dict[str, int]) -> float:
        ...
        has_technical_terms = any(term in answer.lower() for term in 
                                 ['hz', 'frequency', 'rms', 'snr', 'bearing', 'valve', 'gearbox'])
    ...
    def evaluate_single_query(self, query_data: Dict[str, Any], schema: Dict[str, Any]) -> EvaluationResult:
        ...
        try:
            ...
            response = {}
            scores = {"keyword_coverage": 0.0, "answer_quality": 0.0, "performance": 0.0, "overall": 0.0}
            passed = False
    ...
    def run_evaluation(self, golden_dataset: List[Dict[str, Any]], 
                      schema: Dict[str, Any], threshold: float = 0.8) -> QualityGateResults:
        """Run full evaluation suite."""
        logger.info(f"Starting evaluation with {len(golden_dataset)} queries, threshold: {threshold}")
        
        results = []
        for i, query_data in enumerate(golden_dataset, 1):
            logger.info(f"Evaluating query {i}/{len(golden_dataset)}: {query_data['query'][:50]}...")
        ...
        # Apply threshold
        passed_queries = sum(1 for r in results if r.scores["overall"] >= threshold and not r.errors)
...
def main():
    ...
    try:
        ...
        if not results.passed_quality_gate:
            print(f"\n❌ Quality gate FAILED - Score {results.overall_score:.3f} below threshold {results.threshold:.3f}")
            
            # Show failed queries
            failed_results = [r for r in results.individual_results 
