...
try:
    ...
    logging.warning("Visualization libraries not available. Install matplotlib, seaborn, and pandas for full functionality.")
...
class EvaluationReporter:
    ...
    def generate_quality_report(self, results: Dict[str, Any]) -> str:
        ...
        report_lines.append(f"- **Success Rate**: {results.get('passed_queries', 0)}/{results.get('total_queries', 0)} ({results.get('passed_queries', 0)/max(1, results.get('total_queries', 1)):.1%})")
        report_lines.append("")
        ...
        if perf_metrics:
            ...
            report_lines.append(f"- **Average Processing Time**: {perf_metrics.get('avg_processing_time', 0):.2f}s")
            report_lines.append(f"- **P95 Processing Time**: {perf_metrics.get('p95_processing_time', 0):.2f}s")
            report_lines.append(f"- **P99 Processing Time**: {perf_metrics.get('p99_processing_time', 0):.2f}s")
            report_lines.append(f"- **Max Processing Time**: {perf_metrics.get('max_processing_time', 0):.2f}s")
            report_lines.append("")
        ...
        if individual_results:
            ...
            report_lines.append(f"- **Score Statistics**:")
            report_lines.append(f"  - Mean: {statistics.mean(scores):.3f}")
            report_lines.append(f"  - Median: {statistics.median(scores):.3f}")
            report_lines.append(f"  - Std Dev: {statistics.stdev(scores) if len(scores) > 1 else 0:.3f}")
            report_lines.append(f"  - Min: {min(scores):.3f}")
            report_lines.append(f"  - Max: {max(scores):.3f}")
            report_lines.append("")
            
            # Failed queries
            failed_queries = [r for r in individual_results if r["scores"]["overall"] < threshold or r.get("errors")]
            if failed_queries:
    ...
    def generate_benchmark_report(self, results: Dict[str, Any]) -> str:
        ...
        if "performance" in results:
            ...
            report_lines.append(f"- **Requests Completed**: {perf['requests_completed']}/{perf['requests_sent']}")
            report_lines.append("")
        ...
        if "stress" in results:
            ...
            report_lines.append(f"- **Breaking Point**: {stress['breaking_point'] or 'Not reached'}")
            ...
            report_lines.append(f"- **Success Rate**: {stress['successful_requests']/stress['total_requests']:.1%}")
            report_lines.append("")
    ...
    def create_quality_visualizations(self, results: Dict[str, Any]) -> List[Path]:
        ...
        axes[0, 0].axvline(results.get("threshold", 0.8), color='red', linestyle='--', label='Threshold')
        ...
        corr_data = df[["overall", "keyword_coverage", "answer_quality", "semantic_similarity"]].corr()
        ...
        categories = ["Keyword\nCoverage", "Answer\nQuality", "Semantic\nSimilarity"]
        values_top = [top_query["keyword_coverage"], top_query["answer_quality"], top_query["semantic_similarity"]]
        
        angles = np.linspace(0, 2 * np.pi, len(categories), endpoint=False).tolist()
        values_top += values_top[:1]  # Complete the circle
        angles += angles[:1]
        
        axes[0].plot(angles, values_top, 'o-', linewidth=2, label=f'Score: {top_query["overall"]:.3f}')
        ...
        values_worst = [worst_query["keyword_coverage"], worst_query["answer_quality"], worst_query["semantic_similarity"]]
        values_worst += values_worst[:1]
        
        axes[1].plot(angles, values_worst, 'o-', linewidth=2, color='red', label=f'Score: {worst_query["overall"]:.3f}')
    ...
    def create_benchmark_visualizations(self, results: Dict[str, Any]) -> List[Path]:
        ...
        if "performance" in results:
            perf = results["performance"]
            metrics = ["Throughput\n(RPS)", "Avg Response\nTime (s)", "P95 Response\nTime (s)", "Error Rate\n(%)"]
            values = [perf["throughput_rps"], perf["avg_response_time"], perf["p95_response_time"], perf["error_rate"] * 100]
            
            bars = axes[0, 0].bar(metrics, values, color=['lightblue', 'lightgreen', 'orange', 'lightcoral'])
        ...
        if "stress" in results:
            ...
            bars = axes[1, 0].bar(throughput_labels, throughput_values, color=['lightblue', 'darkblue'])
            ...
            values = [stress["successful_requests"]/stress["total_requests"], stress["stability_score"]]
    ...
    def generate_html_report(self, results: Dict[str, Any], charts: List[Path], 
                           report_type: str = "quality") -> Path:
        ...
        if report_type == "quality":
            ...
            <div class="metric">
                ...
                <strong>Success Rate:</strong> {results.get('passed_queries', 0)}/{results.get('total_queries', 0)}
            </div>
            """
    ...
    def generate_comprehensive_report(self, results_path: Path, report_type: str = "quality") -> Dict[str, Path]:
...
def main():
    ...
    parser.add_argument("--type", choices=["quality", "benchmark"], default="quality",
