...
class RAGBenchmark:
    ...
    def run_performance_benchmark(self, queries: List[str], iterations: int = 10) -> BenchmarkResult:
    ...
    def run_stress_test(self, queries: List[str], duration: int = 60, 
                       max_concurrent: int = 50) -> StressTestResult:
        """Run stress test with increasing load."""
        logger.info(f"Running stress test: {duration}s duration, max {max_concurrent} concurrent users")
        ...
        for concurrent_users in range(1, max_concurrent + 1, 5):
            ...
            batch_error_rate = (batch_requests - batch_successful) / batch_requests if batch_requests > 0 else 0
            ...
            logger.info(f"Concurrent users: {concurrent_users}, Throughput: {batch_throughput:.2f} RPS, "
        ...
        stability_score = 1.0 if breaking_point is None else max(0, (breaking_point - 1) / max_concurrent)
    ...
    def run_latency_benchmark(self, queries: List[str], samples: int = 100) -> Dict[str, Any]:
        ...
        return {
            ...
            "percentiles": {
                f"p{p}": percentile_values[int(p * 10 - 1)] if p * 10 - 1 < len(percentile_values) else max(response_times)
...
def main():
    ...
    parser.add_argument("--suite", choices=["performance", "stress", "latency", "all"],
                       default="performance", help="Benchmark suite to run")
